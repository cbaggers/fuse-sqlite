...
public extern(iOS) static class SQLiteImpl {
    ...
    [Foreign(Language.ObjC)]
    public static extern void QueryImpl(QueryRes result,
                                        IntPtr db,
                                        string statement,
                                        string[] param)
    @{
        ...
        while((sqlite_ret = sqlite3_step(compiledStatement)) == SQLITE_ROW)
        {
            ...
            for (int i=0; i<totalColumns; i++)
            {
                ...
                if (dbDataAsChars != NULL)
                {
                    ...
                    @{QueryRes:Of(result).SetElem(string,string):Call([columnNames objectAtIndex:i], [NSString stringWithUTF8String:dbDataAsChars])};
                }
                else
                {
                    @{QueryRes:Of(result).SetElem(string,string):Call([columnNames objectAtIndex:i], nil)};
                }
